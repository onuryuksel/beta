    ...
    if process_button:
        ...
        if competitor_name_live == "Level Shoes":
            ...
            def _prepare_ounass_df(raw_df: pd.DataFrame) -> pd.DataFrame:
                ...
                df = df[df['Count'] > 0]
                if not df.empty:
                    df['Brand_Cleaned'] = df['Brand'].apply(clean_brand_name)
                return df
            # -----------------------------------------------------------
            ...
            with st.spinner(f"Processing Ounass {ctry_b} …"):
                ...
                if html_b:
                    ...
                    df_b  = _prepare_ounass_df(pd.DataFrame(raw_b))
        ...
        df_comparison_sorted_live = st.session_state.get("df_comparison_sorted")
        live_competitor_name      = st.session_state.competitor_selection
        display_all_results(
            df_ounass_live,
            df_competitor_live,
